...
while ($row = mysql_fetch_assoc($result)) {
	echo "<table class='section'><tr class='beast'><td width='20%'><img src='/font/" . $row["sin"] . ".png' title='" . $sins[$row["sin"]] . "'> " . $row["level"] . "</td><td>" . strtoupper($row["name"]) . "<td width='20%'>" . $row["size"] . " <img src='/font/S.png' title='Size'></td></tr></table>";
...
function ability($x) {
	global $ab_id, $row;
	echo "<td style='background:url(missing.png); background-size: 64px 64px; width:64px; height:64px;'><div style='background:url(/creatures/$ab_id/$x.jpg); background-size: 64px 64px; width:64px; height:64px;'></div></td>";
	echo "<td><u>" . $row[$x] . "</u><br>" . $row["$x info"] . "</td></tr>";
} echo "<table style='margin-top:-10px; margin-bottom:-10px;' class='section abilities'><tr>";
$ability = array("alpha", "beta", "gamma", "delta", "epsilon", "omega");
while ($row = mysql_fetch_assoc($result)) foreach ($ability as $x) ability($x);
echo "</tr></table>";
mysql_free_result($result);
