...
export class Hex {
	 ...
	 * Fixed version: Uses screen-space overlap detection instead of hardcoded row scan
	...
	ghostOverlap() {
		...
		// Check all creatures for screen-space overlap
		game.creatures.forEach((creature) => {
			if (!creature || creature === this.creature) {
				return; // Skip self or invalid creatures
			}
			
			// Only consider creatures that could be in front (higher y = closer to camera)
			// OR on the same row but rendered in front based on display order
	...
	/**
	 * Get screen-space bounding box for this hex
	 */
	getScreenBounds(): { left: number; right: number; top: number; bottom: number } {
		const width = this.width || Const.HEX_WIDTH_PX;
		const height = this.height || Const.HEX_HEIGHT_PX;
		
		return {
			left: this.displayPos.x + 8,      // Small inset for better feel
			right: this.displayPos.x + width - 8,
			top: this.displayPos.y + 6,
			bottom: this.displayPos.y + height + 12,  // Extend slightly to catch feet
		};
	}
	 ...
	 */
	boundsOverlap(a: any, b: any): boolean {
