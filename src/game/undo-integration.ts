...
import { ActionHistory, ActionType } from './game/action-history';
...
export class UndoIntegration {
  ...
  public beforeAction(
    ...
    const playerId = this.game.currentPlayerId || 'unknown';
    
    this.undoManager.beforeAction(
      type,
      playerId,
      actorId,
      targetId,
  ...
  public getUndoStatus(): {
    ...
    return this.undoManager?.getUndoStatus() || {
      available: false,
      usedThisRound: false,
      canUndoAgain: false,
      cooldownRemaining: 0
 ...
 */
export function integrateUndo(game: Game, config?: Partial<UndoIntegrationConfig>): UndoIntegration {
