...
var Creature = Class.create({
		...
		var interval = setInterval(function() {
			if (!G.freezedInput) {
				...
				if (G.turn >= G.minimumTurnBeforeFleeing) {
					G.UI.btnFlee.changeState("normal");
				}
				G.startTimer();
				crea.queryMove();
			}
	...
	queryMove: function(o) {
		...
		var selectNormal = function(hex, args) {
	...
	previewPosition: function(hex) {
		...
		crea.tracePosition({
			x: hex.x,
			y: hex.y,
			overlayClass: "hover h_player" + crea.team
		});
	},
	...
	tracePath: function(hex) {
		...
		creature.tracePosition({
			x: last.x,
			y: last.y,
			overlayClass: "creature moveto selected player" + creature.team
		});
	...
	hint: function(text, cssClass) {
		...
		this.hintGrp.forEach(function(grpHintElem) {
			if (grpHintElem.cssClass == 'confirm') {
				...
				grpHintElem.tweenAlpha = G.Phaser.add.tween(grpHintElem).to({
					alpha: 0
		...
		this.hintGrp.forEach(function(grpHintElem) {
			...
			grpHintElem.tweenPos = G.Phaser.add.tween(grpHintElem).to({
				y: offset
			}, tooltipSpeed, tooltipTransition).start();
		}, this, true);
	...
	die: function(killer) {
		...
		// Kill animation
		var tweenSprite = G.Phaser.add.tween(this.sprite).to({
			alpha: 0
		}, 500, Phaser.Easing.Linear.None).start();
		var tweenHealth = G.Phaser.add.tween(this.healthIndicatorGroup).to({
			alpha: 0
		}, 500, Phaser.Easing.Linear.None).start();
		tweenSprite.onComplete.add(function() {
			crea.sprite.destroy();
		});
		tweenHealth.onComplete.add(function() {
			crea.healthIndicatorGroup.destroy();
		});
