...
var Ability = Class.create({
	initialize: function(creature, abilityID) {
		...
		this.token = 0;
		var data = G.retreiveCreatureStats(creature.type);
		$j.extend(true, this, G.abilities[data.id][abilityID], data.ability_info[abilityID]);
	...
	areaDamage: function(attacker, damages, effects, targets, ignoreRetaliation) {
		...
		if (multiKill > 1) attacker.player.score.push({
			type: "combo",
			kills: multiKill
		});
	},
