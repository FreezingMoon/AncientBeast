...
import { ObjectStats, ScoreArrayType, StatusEffects } from './CreatureTypes';
...
export class Creature {
	...
	*/
	...
	/*
	 * Just taking the logic from comments and implementing in typescript, same format
	*/

	// Normal attributes
	x: number; //Hex Coordinates for X
	y : number; //Hex Coordinates for Y
	pos: Object; //Pos object for hex comparison {x,y}
	
	name: string;  //Creature Name
	id: number; //Creature ID increamenting, starting at 1
	size: number; //Creature size in hexes: 1, 2, or 3
	type: string; //Type of the creature in the database
	team: number; //Owner's ID (0-3)
	player: Player; //Player Object
	hexagons: Array<Hex>; //Array containing the hexes of the creature's location
	level: number; //State of the current level
	_movementType: string; //Represents the movement of a creature


	dead: boolean; //If Dead - True
	undead: boolean; //If Undead - True
	stats: ObjectStats; //Object containing stats of the creature
	statsAlt: Object; // Object containing the alteration value for each stat //todo
	abilities: Array<Ability>; //Array containing the 4 abilities
	remainingMove: number; //Remaining moves allowed until the end of the turn
	temp: boolean; // True if the creature is only temporary for preview
	realm: any; //Any Realm in the database
	animation: Animation; //Shows the animation effect
	display: any; //Shows the Display for the character
	drop: any; //Shows the dropped object
	delayed: boolean; //True if the Creature is currently delayed
	delayable: boolean; //True if the Creature can even be delayed
	noActionPossible: boolean; //True if the Creature has nothing else to do
	materializationSickness: boolean; //True if the Creature can have sickness effect
	score: Array<ScoreArrayType> //Total Score Object the creature has acquired
	playable: boolean; //If the user can select the current creature
	ability_info: Array<Ability>; //Any array that contains each ability
	...
	hasWait: boolean; //If there's a wait
 	travelDist: number; //Distance Creature has moved
	effects: Array<Effect> //Array of current effects on creature
	killer: Creature; //Which Creature is the killer
	dropCollection: Array<Drop> //Array containing all the drops of a creature
	protectedFromFatigue: boolean; //If it's a dark Priest set to true
	turnsActive: number; //Current turns left to play
	endurance: number; //Endurance stat on a creatures
	health: number; //Amount of health the current creature has 
	...
	baseStats: ObjectStats; //Object containing all the base stats of any object
	status: StatusEffects;

	//Group Types
	grp: Group; // The current group of a creature
	hintGrp: Group; //Another group object, but it hints other users
	healthIndicatorGroup: Group //Shows the health bars of the group

	//Sprite Types
	sprite: Sprite; //The current picture of the object
	healthIndicatorSprite: Sprite; //Sprite of the current creatures health

	//Text Types
	healthIndicatorText: Text | any; //Displays text of the health indicator
	fatigueText: Text | string; //Shows other users if character is fatigue
	 ...
	 */
	summon(disableMaterializationSickness:boolean = false) {
		let game = this.game;
	...
	activate() {
		...
		let varReset = function (this: any) {
	...
	/* deactivate(wait)
	 *
	 * wait :	Deactivate while waiting or not
	 *
	...
	queryMove(o) {
		...
		let selectNormal = function (hex: Hex, args) {
			args.creature.tracePath(hex);
		};
		let selectFlying = function (hex: Hex, args) {
	 ...
	 *
	 * facefrom : Hex to face from
	 * faceto : Hex to face
	 *
	 * Face creature at given hex
	 *
	 */
	faceHex(faceto: Hex | Creature, facefrom: Hex | Creature, ignoreCreaHex: boolean, attackFix: boolean) {
		if (!facefrom) {
			facefrom = this.player.flipped ? this.hexagons[this.size - 1] : this.hexagons[0];
		}

		if(faceto instanceof Hex && facefrom instanceof Hex){			
			if (
				ignoreCreaHex && 
		...
		if (attackFix && this.size > 1) {
			...
			if (facefrom.y % 2 === 0) {
				if (faceto.x - Number(this.player.flipped) == facefrom.x) {
					this.facePlayerDefault();
					return;
				}
			} else {
	 ...
	 *
	 * return :	Array containing the path hexes
	 *
	 */
	calculatePath(x: number, y: number) : Array<Hex> {
		let game = this.game;
	...
	/* heal(amount)
	 *
	 * amount :	Amount of health point to restore
	 * isRegrowth: Determines if there should be health regeneration
	 */
	heal(amount: number, isRegrowth: boolean, log:boolean = true) {
		let game = this.game;
		// Cap health point
		...
		if (amount > 0) {
			...
			if (isRegrowth) {
				...
				this.hint(String(amount), 'damage d ' + amount);
			}
	...
	takeDamage(damage: any, o:any) {
		...
		damage.melee = false;
		this.adjacentHexes(1, null).forEach((hex) => {
		...
		if (damage.status === '') {
			...
			this.hint(String(nbrDisplayed), 'damage d' + dmgAmount);
			...
			damage.effects.forEach((effect: Effect) => {
				this.addEffect(effect, null, null);
			});
	 ...
	 *
	 */
	addEffect(effect: Effect, specialString:string, specialHint, disableLog:boolean = false, disableHint:boolean = false) {
		let game = this.game;
			...
			if (!disableHint) {
				if (specialHint || effect.specialHint) {
					this.hint(specialHint, 'msg_effects');
						...
						this.hint(String(new Text(effect.name)), 'msg_effects');
				}
			}
	...
	updateAlteration() {
		...
		buffDebuffArray.forEach((buff:Drop) => {
			$j.each(buff.alterations, (key, value:any) => {
	 ...
	 *
	 * killer :	Killer of this creature
	 *
	 */
	die(killer:Creature) {
		let game = this.game;
		...
		this.killer.player = killer.player;
		...
		//The unused datatypes are null so that they may be changed later
