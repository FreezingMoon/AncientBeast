...
export class Ability {
	...
	movementType: string; //The way this character moves
	 ...
	 */
		...
		if (this.getTrigger() === 'onQuery' && !deferredEnding) {
			game.activeCreature.queryMove(this.creature);
		}
	}
	...
	animation2(o) {
		...
		if (args[0]) {
			if (args[0] instanceof Creature) {
				this.creature.faceHex(args[0], this.creature, true, true);
			} else if (args[0] instanceof Array) {
				for (let argument of args[0]) {
					if (argument instanceof Creature || argument.creature) {
						this.creature.faceHex(argument, this.creature, true, true);
					}
				}
			}
		}
