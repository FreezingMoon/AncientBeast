...
export class P2PHandler {
	...
	private peerDisconnectedHandler?: (peerId: string) => void;
	private messageReceivedHandler?: (message: P2PMessage) => void;
	...
	public sendMessage(peerId: string, message: P2PMessage): void {
		...
		if (connection.dataChannel.readyState === 'open') {
			...
			console.warn(
				`Data channel not ready for peer ${peerId}, state: ${connection.dataChannel.readyState}`,
			);
		}
	}
