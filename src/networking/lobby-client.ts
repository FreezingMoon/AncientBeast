...
export class LobbyClient {
     ...
     * Create a new lobby
     */
    public async createLobby(name: string = '', isPrivate: boolean = false, maxPlayers: number = 8): Promise<LobbyInfo> {
        if (!this.playerId) {
     ...
     */
    public async addToQueue(gameMode: string = 'standard'): Promise<number> {
        if (!this.playerId) {
            throw new Error('Player not registered');
        }
     ...
     * Get server stats
     */
    public async getStats(): Promise<{ players: number; lobbies: number; queuedPlayers: number; uptime: number }> {
